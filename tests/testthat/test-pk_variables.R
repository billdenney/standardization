test_that("pk_variables works ", {
  pk_var <-
    suppressWarnings(pk_variables(data.frame(AVALC=c("<2.30","3.45","BLQ","BQL<2.1",
  ...
  expect_setequal(check_AVALN %in% c(0.00 ,3.45 ,  NA),TRUE)
  expect_setequal(check_ALLOQ %in% c("2.30" ,""   ,  NA   ,  "2.1",  "1.3" , "1.5" ),TRUE)
  expect_setequal(check_AULOQ %in% c(""  ,   "1000", "2000"),TRUE)
